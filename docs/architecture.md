...
Mainnetのユーザー入金はUSDCのみ、またはWHYPEのみになり得ます。集中流動性のアクティブレンジに入れるには両側トークンが必要なため、`HyperpoolVault` は片側入金時に以下を実行します。

1. Project X pool `slot0` から現在価格を取得
2. 入金額の一部を `swapRouter.exactInputSingle` で反対側トークンへswap
3. USDC + WHYPE を `ProjectXAdapter` へ転送
4. Adapter がProject X NPMでレンジ付きLPをmint

この処理により、初回入金でも固定価格 `42 USDC/HYPE` ではなく、Project X実勢価格付近のレンジでLPが作成されます。

### 価格スケール

コントラクト内の基準価格 `priceUsdc6PerHype18` は **human price × 1e18** です。例: `61.2 USDC/HYPE` は `61.2e18`。

| 入力 | スケール | 変換 |
|------|----------|------|
| Project X pool `slot0` | V3 raw sqrt price | `ProjectXAdapter.currentPoolPriceUsdc6PerHype18()` |
| HyperCore oracle `oraclePx` | Mainnet実値は4桁小数 | `px * 1e14` |
| HYPE→USDC評価 | WHYPE 18 decimals / USDC 6 decimals | `hypeAmount * price / 1e30` |
| USDC→HYPE評価 | USDC 6 decimals / WHYPE 18 decimals | `usdcAmount * 1e30 / price` |

HyperCore oracle は `HyperpoolVault.rebalance()` の安全装置です。`refPrice` が oracle から `maxRebalanceDeviationBps`（標準5%）を超えるとrevertします。
...
2. `collect` でNPM内にcreditされた元本をAdapterへ回収
3. 参照価格と +10% / −30% bps から新 tick を計算
4. 手元トークンで `npm.mint` → **新しい `tokenId` を発行**
5. `positionTokenId` を新 ID に更新

rebalance時のNPM mintは `amount0Min/amount1Min = 0` です。価格変更で必要な資産比率が変わるため、片側が余ることがあります。余ったトークンはAdapter残高として残り、Vaultの `totalAssetsUsdc()` に含まれます。
...
2. **残骸 NFT に資金なし** — 旧 tokenId の `liquidity == 0`、かつwithdraw/rebalance時に `collect` 済みであること
...
5. **oracleスケール** — MainnetのHyperCore oracle価格とProject X pool価格が同じ桁で近接していること
