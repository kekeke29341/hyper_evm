...
- 取引手数料の **70%** を USDC Cashdrop、**30%** を運営
...
| エアドロップ対象者 | Cashdrop（Merkle 証明）で USDC を請求（JST 7–9） |
| 運営 / keeper | リバランス cron、日次 harvest + Merkle 設定 |
...
| **Vault deposit** | 運営またはユーザーが LP を開始 |
...
| ユーザー還元 | USDC 手数料の **70%**（Vault シェア比例 · Cashdrop） |
| 運営 | USDC 手数料の **30%** |
