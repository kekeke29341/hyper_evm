...
| **Cashdrop** | 毎朝 JST 7:00、collect 手数料の **60%** を USDC で対象ユーザーへ自動送金 |
...
- **収益分配**: collect した LP 手数料の **7% 運営 / 60% ユーザー / 33% オーナー**（日次 USDC）
- APR 表示: GeckoTerminal のプール実績から自動計算（10分キャッシュ）+ 脚注「実質還元 ≒ プール APR × 60%」
...
| 運営取り分 | collect 手数料の **7%**（別途オーナー **33%**） |
| ユーザー還元 | collect 手数料の **60%**（Vault シェア比例、JST 7:00 USDC 自動送金） |
...
3. **7%** → 運営ウォレット、**33%** → オーナーウォレット（USDC）
4. **60%** → Vault シェア比例で対象者リストを作成
