...
| **Rewards** | Merkle ルート生成・設定、資金投入、pause、期限後の回収 |
...
1. 対象者リストを CSV で用意（1行 = `address,amount`、USDC は **6 decimals** の整数）
   ...
   ```
2. Admin → **Rewards** → CSV を貼り付け → **Preview Root** で検証
...
|------|----------|--------------|
| Merkle ルート計算 | Rewards タブ（`merkle.ts` と同じロジック） | `frontend/src/lib/admin/merkle.ts` |
