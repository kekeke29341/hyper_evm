...
| 役割 | コントラクト | 権限の例 |
|------|-------------|---------|
| **Factory Admin** (`feeToSetter`) | `ProjectXFactory` | `createPair`, `syncPairs`, `setFeeCollector`, `setTrustedRouter`, `setFeeToSetter` |
| **Points Owner** | `PointsDistributor` | `authorizePool` / `deauthorizePool` |
| **Airdrop Owner** | `MerkleAirdrop` | `setMerkleRoot`, `fund`, `pause` / `unpause`, `recoverUnclaimed` |
| **Vault Owner** | `HyperpoolLiquidityVault` | `setKeeper`, `setTargetRangeBps`, `pause` / `unpause` |
| **Referral** | `ReferralRegistry` | ユーザー自身が `registerCode`（Admin UI からの一括登録 API はなし） |
...
1. `./scripts/deploy-testnet.sh` でコントラクトデプロイ
2. `node scripts/sync-abi.mjs` → フロントの `deployments/998.json` 同期
3. Admin → **Pools** → Primary Pair が Points **未 authorize** なら「Authorize Primary Pair」
4. （任意）流動性シード: `SEED_LIQUIDITY=true ./scripts/deploy-testnet.sh`
5. フロントを Vercel にデプロイ（`NEXT_PUBLIC_DEFAULT_CHAIN_ID=998`）
...
- ユーザーは **Position** タブから Zap / Withdraw
- Vault Owner は **Vault** タブで pause、keeper 設定、表示用 `targetRangeBps` を変更
- keeper は `rebalance` 等の keeper 専用操作向け（将来拡張）
...
| オンチェーン検証 | Analytics タブ | `node frontend/scripts/testnet-verify-onchain.mjs` |
...
- [ ] Primary pair が Points authorize 済み
