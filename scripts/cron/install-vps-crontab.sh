...
#
# Locks live under HYPERPOOL_LOCK_DIR (default /opt/hyperpool/locks). Do NOT use /var/lock on
# WSL — flock open fails there with Permission denied and silently skips jobs.
#
# Usage:
...
LOCK_DIR="${HYPERPOOL_LOCK_DIR:-/opt/hyperpool/locks}"
...
chown hyperpool:hyperpool "$LOCK_DIR" 2>/dev/null || true
chmod 775 "$LOCK_DIR" 2>/dev/null || true
for pool in legacy upump-whype ubtc-whype ueth-whype; do
  for kind in harvest distribute keeper; do
    f="$LOCK_DIR/hyperpool-${kind}-${pool}.lock"
    touch "$f"
    chown hyperpool:hyperpool "$f" 2>/dev/null || true
...
0 7 * * * TZ=Asia/Tokyo flock -n $LOCK_DIR/hyperpool-harvest-legacy.lock $HARVEST >> $LOG_DAILY 2>&1
0 9 * * * TZ=Asia/Tokyo flock -n $LOCK_DIR/hyperpool-distribute-legacy.lock $DISTRIBUTE >> $LOG_DAILY 2>&1
30 9 * * * TZ=Asia/Tokyo flock -n $LOCK_DIR/hyperpool-distribute-legacy.lock $DISTRIBUTE >> $LOG_DAILY 2>&1
0 */6 * * * flock -n $LOCK_DIR/hyperpool-keeper-legacy.lock $KEEPER >> $LOG_KEEPER 2>&1
EOF
)
fi
